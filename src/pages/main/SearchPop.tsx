...
const SearchPop = (props) => {
  ...
  // console.log(process.env.REACT_APP_API_KEY_NR)
  ...
  return (
    <div className='search-pop'>
      ...
      <div className="hospital-wrap">
        {
          callHospital.map((v:any,i)=>{
            return(
              <div className="hospital" key={i} onClick={()=>{
      ...
      <div className="button-box">
        <button className='prev button' onClick={()=>{setChNum(--chNum)}} 
        ...
        <button className='next button' style={{background: `${
          chNum === Math.ceil(totalCount / 10) -1 ? '#DEE9FE' : '#fff'}`
        }}><IoIosArrowForward className='arrow' onClick={()=>{setChNum(++chNum)}}/></button>
      </div>
    </div>
  )
}
