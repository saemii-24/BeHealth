...
const ExercisePop = (props) => {
  ...
  const handleSelect = (e) => {
    ...
    for(let i = 0; i < ageList.current.length; i++) {
      ageList.current[i].style.background = "#fff";
        ...
        <div className='classify'>
          {
            recommendExercise.map((v,i)=>{
              return(<button key={i} ref={el => ageList.current[i] = el} onClick={(e)=>{handleSelect(e)}}>{v.age}</button>)
            })
          }
        </div>
        ...
        <div className='classify'>
          {
            exerciseCategory.map((v,i)=>{
              return(<button key={i}>{v.category}</button>)
            })
          }
        </div>
      </div>
