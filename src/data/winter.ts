...
let winter: SeasonType[] = [
  {
    ...
    commonPop: [
      ...
      {
        explain: `식중독은 여름에만 주의를 기울이면 되는 질병으로 생각하기 쉽지만 그렇지 않다. 겨울철 기승을 부리는 노로바이러스 때문이다. 
        노로바이러스는 심한 구토와 설사 증상으로 몸과 마음을 괴롭게 한다. 
        건강하고 안전하게 겨울을 날 수 있도록 노로바이러스 위험요소 및 예방수칙과 함께 대처법을 살펴본다.`,
        info: [
          {
            name: '제철 맞은 음식도 생으로 먹으면 위험해요',
            content: `겨울은 굴이나 조개, 방어 등 다양한 제철 수산물을 맛볼 수 있는 계절이다. 특히 날것 그대로 섭취하는 경우가 많은데 수산물을 익히지 않고 생으로 먹을 경우 노로바이러스에 걸릴 확률이 높기 때문에 주의가 필요하다.`,
            content2: `노로바이러스는 급성장염을 일으키는 매우 작은 바이러스로 연중 내내 조심해야 하지만 그중에서도 기온이 낮은 겨울에 더 활발하게 움직이기에 주의가 필요하다.\n
            소량의 바이러스로도 감염될 만큼 전염성이 높아 겨울철 식중독의 주범으로 손꼽힌다.`,
            content3: `밀폐된 공간에 머무는 시간이 길어지는 겨울철이 면 사람 간 전파력 또한 높아져 어린이집, 학교, 요양원 등에서 집단 식중독을 일으키기도 한다.\n
            질병관리청 발표에 따르면 노로바이러스는 지난 3년간 12월에서 1월 사이에 많이 발생한 것으로 조사됐다.`
          },
          {
            name: '독감 증상과 비슷해 헷갈릴 수 있어',
            content: `대표적인 수인성 식품매개 질환인 노로바이러스는 오염된 음식이나 물을 섭취했을 때 감염되기 쉽다.\n 
            오염된 음식을 통해 들어온 바이러스는 소장, 대장 등에서 증식하며 장 점막에 침투해 염증을 일으킨다.`,
            content2: `로바이러스에 감염되면 1~2일 잠복기를 거친 후 구토, 설사 증상이 2~3일간 지속된다.\n
            시간이 지나면 대부분 저절로 회복되지만 노인이나 소아, 영유아는 합병증이 나타날 수 있어 병원에 내원하는 것이 좋다.\n
            특히 아이들은 수분 보충이 잘 되지 않으면 탈수증이 나타날 수 있기에 물이나 이온음료로 수분공급을 충분히 해주고, 심한 탈수가 생겼을 경우 병원에서 정맥을 통해 수액을 공급해야 한다.`,
            content3: `구토와 설사 외에 메스꺼움, 탈수, 복통, 근육통, 두통 등의 증상 때문에 자칫 독감으로 오인할 수 있기에 증상이 계속될 경우 병원을 찾아 정확한 진단을 받는 것이 좋다.`
          },
          {
            name: '영하 20도에도 살아남는 노로바이러스 예방법',
            content: `종류만 무려 150여 종에 달하는 노로바이러스는 감염 후에도 면역반응이 오래가지 않아 재감염될 가능성이 있다.\n 
            또한 60도에 30분 동안 가열해도 감염성이 유지되며 심지어 영하 20도에서도 생존이 가능한 것으로 알려져 있다.`,
            content2: `때문에 예방을 위해 되도록 생식을 삼가고, 음식을 만들 때는 먼저 손을 30초 이상 씻고 조리도구는 끓는 물에 소독해 사용한다.\n
            음식은 100도에서 1분 이상 가열하는 것이 좋다.\n
            이 외에도 감염자가 사용한 화장실이나 문고리, 물품 등은 소독 후 이용하며, 감염자와의 만남을 금하는 것이 예방에 도움이 된다.`,
            content3: `집단배식을 하는 조리사나 간호직 종사자라면 구토, 설사 등의 증상이 멈췄더라도 3일 이후까지 업무를 하지 않는 것이 바람직하다.\n
            설사 증세를 보이는 유아 기저귀를 만진 경우, 환자와 간접 접촉한 경우에도 물건을 통해 쉽게 전파될 수 있으므로 주변에 노로바이러스 환자가 있다면 환경을 청결히 관리해야 한다.`
          },
          {
            name: '노로바이러스 환자 가족 및 동거인 수칙',
            content: `1. 환자가 사용한 화장실은 염소(1,000~5,000ppm)를 묻혀 10분간 방치해 소독한다.\n 
            2. 환자가 만졌거나 구토물에 오염된 식품은 폐기하고 문고리나 물품은 소독해 사용한다.\n 
            3. 식기는 세제를 묻혀 온수로 씻고, 빨래는 세제를 사용해 세탁기로 세탁한다.\n
            4. 다른 사람들의 방문을 자제하고 완쾌한 후 청소와 소독을 했더라도 3일 후에 방문한다.\n
            5. 환자는 공간을 분리해 혼자 지낼 것을 권장하며 수건은 각자 따로 사용한다.`,
          },
        ],
      },
    ],
